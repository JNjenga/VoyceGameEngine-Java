...
public class NuklearManager {
	...
	public NuklearManager() {
		
 		try {
            this.ttf =  ioResourceToByteBuffer("SourceCodePro.ttf", 512 * 1024);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
		
		ctx = setUpWindow(Window.window);
		widgets = new HashMap<>();
		setUpFonts();
 	}
	...
	public void addWidgets(Widget ... widget ) {
		for (Widget wid : widget) {
			widgets.put(wid.getWindowName(),wid);
		}
 	}
	
	public void addWidgets(Widget  widget ) {
		widgets.put(widget.getWindowName(),widget);
 	}
    ...
    public void newFrame() {
        ...
        if (mouse.grab()) {
            ...
            glfwSetCursorPos(Window.window, prevX, prevY);
            mouse.pos().x(prevX);
            mouse.pos().y(prevY);
	...
	public void render( ) {
		...
		widgets.forEach((name,widget)->{widget.draw(ctx);});
    ...
    private void render(int AA, int max_vertex_buffer, int max_element_buffer) {
...

        {
            ...
            ByteBuffer vertices = Objects.requireNonNull(
	...
	private static ByteBuffer resizeBuffer(ByteBuffer buffer, int newCapacity) {
	 ...
	 public static ByteBuffer ioResourceToByteBuffer(String resource, int bufferSize) throws IOException {
	        ...
	        Path path = Paths.get("tests/res/fonts/SourceCodePro.ttf");
	         if (Files.isReadable(path)) {
	            ...
	            try {
	                InputStream source = NuklearManager.class.getClassLoader().getResourceAsStream(resource);
